    ...
    @pushonce('script')
        <script>
            ...
            save_btn.onclick = function () {
                save_btn.style.backgroundColor = '#000';
                save_btn.textContent = 'Saving...';



                // let xhr = new XMLHttpRequest();
                // xhr.open('POST', '/save-pin', true);
                // xhr.setRequestHeader('Content-Type', 'application/json');
                // xhr.setRequestHeader('X-CSRF-TOKEN', document.querySelector('meta[name="csrf-token"]').getAttribute('content'));
                //
                // // You can send data here, like form values
                // let data = JSON.stringify({data: 'exampleData'});
                // xhr.send(data);

                // Handle the response
                // xhr.onload = function () {
                //     if (xhr.status === 200) {
                //         save_btn.textContent = 'Saved!';  // Success response
                //     } else {
                //         save_btn.textContent = 'Failed';  // Error handling
                //         save_btn.style.backgroundColor = '#f44336';  // Error color
                //     }
                // };
            };
        </script>
